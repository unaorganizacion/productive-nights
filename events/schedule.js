// schedule.js
// const Promise = require("bluebird");
const sendMessage = require('../tools/sendMessage');
const moment = require('moment-timezone'),waterfall = require("async/waterfall");

let event = function (userObject) {
  this.userObject = userObject;
};

event.prototype = {};

event.prototype.setDatastore = function (datastore) {
  this.datastore = datastore;
};

event.prototype.setArguments = function (args) {
  this.arguments = args[1];
  //this.extraArgument = args[2]; 'Ofertas'
}

event.prototype.run = function () {
  let
    categoriesIds = this.arguments.split(','),
    datastore = this.datastore,
    userObject = this.userObject
  ;
  
  function fromDatastore (obj) {
        let newObj = obj.messageData;
        newObj.id = obj[datastore.KEY].id;
        return newObj;
    }

    return new Promise((resolve, reject) => {
        let
            posts = [],
            postsIds = []
        ;

        function checkDup (postEntity) {
            let dup = false;
            if (postsIds.indexOf(postEntity.id) !== -1) dup = true;
            return dup;
        }

        function createWaterfallFunction (category) {
            return function createWaterfallFunctionResult (cb) {
                let query = datastore.createQuery("Post")
                    .filter('categories', '=', parseInt(category))
                    .filter('day', '=', moment().tz("America/Chihuahua").format('dddd').toLowerCase())
                ;

                query.run((err, entities, info) => {
                  console.log(category, err, entities, info);
                    if (err) {
                        console.error("Error transation",err);
                        // probably no results
                        cb();
                        return;
                    }

                    for (let entity of entities) {
                        let post = fromDatastore(entity);
                        post.sentDate = entity.sentDate;

                        if (!checkDup(post)) {
                            posts.push(post);
                            postsIds.push(post.id);
                        } else {
                            console.log('dup or not in range', !checkDup(post));
                        }
                    }
                    cb();
                });
            }
        }

        let functions = [];
        for (let category of categoriesIds) {
            functions.push(createWaterfallFunction(category));
        }

        waterfall(functions, (err, result) => {
            if (posts.length > 0) {
                posts.sort(function(a, b) {
                    var nameA = new Date(a.sentDate); // ignore upper and lowercase
                    var nameB = new Date(b.sentDate); // ignore upper and lowercase
                    if (nameA < nameB) {
                        return 1;
                    }
                    if (nameA > nameB) {
                        return -1;
                    }
                    return 0;
                });

                let postsElements = [];
                let
                    page = 0,
                    elementsProcessed = 0
                ;
                postsElements.push([]);
                for (let postToElement of posts) {
                    delete postToElement.id;
                    delete postToElement.sentDate;
                    postsElements[page].push(postToElement);
                    elementsProcessed++;
                    if (elementsProcessed >= 10) {
                        page++;
                        elementsProcessed = 0;
                        postsElements.push([]);
                    }
                }
              
                function createWaterfallFunctionPostsSets (posts) {
                    return function (cb) {
                        sendMessage.sendObjectMessage(userObject.mId, {
                            attachment: {
                                type: "template",
                                payload: {
                                    template_type: "generic",
                                    elements: posts
                                }
                            }
                        }, cb);
                    }
                }
                let functions = [];
                for (let posts of postsElements) {
                        functions.push(createWaterfallFunctionPostsSets(posts));
                }

                functions.reverse();
              
                waterfall(functions, () => {
                    // you could send here a messege at the end of all the carousels
                });
              
            } else {
                if (err) {
                    console.error(err);
                } else
                    console.error("today offers query did not throwed results", categoriesIds);
                reject();
            }
        });
    });
  
};

module.exports = event;
